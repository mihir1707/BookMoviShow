...
function TrailersSection() {
    ...
    if(!nowShowingMovies.length){
        return <div className="text-white px-4 sm:px-10">Loading trailers...</div>
    }

    return (
        <div className='px-3 sm:px-6 md:px-16 bg-black lg:px-24 xl:px-44 py-10 sm:py-20 overflow-hidden'>
            ...
            <div className='relative mt-4 sm:mt-6 w-full max-w-240 mx-auto border-2 border-white rounded shadow-primary shadow-md'>
                {
                    !isPlaying ? (
                        <div className='relative cursor-pointer rounded-lg overflow-hidden' onClick={() => setIsPlaying(true)} >
                            <img src={nowShowingMovies[currentTrailer].bannerUrl} alt='trailer' className='w-full h-40 sm:h-80 md:h-100 lg:h-135 object-cover brightness-75' />
                            <PlayCircleIcon strokeWidth={1.6} className='absolute top-1/2 left-1/2 w-8 sm:w-12 md:w-16 h-8 sm:h-12 md:h-16 transform -translate-x-1/2 -translate-y-1/2 text-white' />
                        </div>
                    ) : (
                    <ReactPlayer 
